...
std::vector<int> ColorPallette::reduceColors(std::size_t count) {
  ...
  if (count >= totalColors) {
    P_LOG_INFO() << "Color count is less than or equal to requested count, no "
                    "reduction needed"
                 << logger::endl;
    return {};
  }
  ...
  if (count & (count - 1)) {
    ...
    P_LOG_WARN() << "Count is not a power of 2, rounding up to " << count
                 << logger::endl;
  }
...
uint16_t ColorPallette::addColor(const uint8_t red, const uint8_t green,
                                 const uint8_t blue,
