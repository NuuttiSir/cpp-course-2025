...
void shuffleCards() {
  ...
  // The images of ten playing cards are located in Images directory,
  // named 1.jpg - 10.jpg
  ...
  // To properly shuffle the images you will need to use the List object in some
  // way
  List<PNGLayer> list;

  // Load the card images into the list.
  // Your job is to update the lines below to properly populate the list with
  // all the cards, then to properly shuffle them, and then to arrange them on
  // the output canvas in the shuffled order.

  // TODO, update the lines below as needed. Currently, the code works for one
  // layer(card) only.
  ...
  for (int i = 1; i <= num_cards; i++) {
    // Create a new layer for each card
    PNGLayer p(WIDTH, HEIGHT, Vector2(0.0, 0.0), Vector2(1.0, 1.0),
               RGBAPixel(255, 255, 255, 255), std::to_string(i));

    // Read each layer from the corresponding image file
    p.readFromFile(INPUT_FILE + std::to_string(i) + ".png");

    // Insert the layer into the list
    list.insertBack(
        p); // Using insertBack to maintain original order before shuffling
  }

  // Now shuffle the layers
  // You MUST use the shuffle function from the List class (our Autograder will
  // check for it)
  list.shuffle();
  ...
  for (auto it = list.begin(); it != list.end(); ++it, ++index) {
    PNGLayer &card = *it;
    // index should start with 0 to go up to the size of the list according to
    // the shuffle function This ist the way the layers should be ordered in the
    // Canvas object. Set up the position of each layer according to index and
    // dimensions
    card.SetPosition(Vector2(
        WIDTH / 100 + ((WIDTH - WIDTH / 9) / num_cards) * index, HEIGHT / 4));
