...
void brighten(PNG & original, int amount)
{
    ...
    for (size_t yi = 0; yi < original.height(); yi++)
    {
        for (size_t xi = 0; xi < original.width(); xi++){
            int new_r = original(xi, yi).red + amount;
            int new_g = original(xi, yi).green + amount;
            int new_b = original(xi, yi).blue + amount;
            ...
            original(xi, yi).red = new_r;
            original(xi, yi).green = new_g;
            original(xi, yi).blue = new_b;
        }
    }
}
