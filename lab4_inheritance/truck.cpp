...
Truck::Truck(const Vector2& pcenter)
	...
	cabin(Vector2(center_.x()+CABIN_WIDTH/2, (center_.y()+TRAILER_HEIGHT/2)-CABIN_HEIGHT/2), CABIN_COLOR, CABIN_WIDTH, CABIN_HEIGHT),
	window(Vector2(cabin.center().x(), cabin.center().y()-WINDOW_HEIGHT/2), WINDOW_COLOR, WINDOW_WIDTH, WINDOW_HEIGHT),
	engine(Vector2(cabin.center().x()+CABIN_WIDTH/2 + ENGINE_WIDTH/2, (cabin.center().y()+CABIN_HEIGHT/2) - ENGINE_HEIGHT/2), ENGINE_COLOR, ENGINE_WIDTH, ENGINE_HEIGHT)
{
...
{
	const Vector2 delta(pcenter - this->center_);
